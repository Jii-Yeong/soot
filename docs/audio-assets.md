...
- **앞 페이드인 4.25초를 잘라야 한다.** 후보 12개 중 가장 길다. 스테이지 진입 순간
  4초를 무음에 가깝게 흘려보내는 셈이라 우선순위가 가장 높다.
- 뒤 페이드아웃 3.05초도 자른다. 전체 167.5초. BPM 추정이 88/120/122로 갈려 한 마디
  길이를 확정하지 못했으므로 **파형에서 마디 머리를 직접 확인한다.**
- 피크 -0.2dBFS, 클리핑 0샘플. -3dBFS로 노멀라이즈한다.
- 구간 편차 7.8dB는 루프 이음매에서 레벨이 튄다는 뜻이기도 하다. 이음매 단차가 2.0dB로
  측정됐으니 **크로스페이드를 잡을 때 이 지점을 먼저 듣는다.**

그대로 두어도 게임은 정상 동작하므로, 가공본이 나올 때까지 무음을 면하는 용도로 둔다.
...
- `AudioDirector.loadMusic()`이 `fetch` 후 `sound.decodeAudio()`로 캐시에 넣는다
