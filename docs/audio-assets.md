...
| `client/src/assets/audio/sources/` | city 원본 하나만 남아 있는 **잔재.** 새 원본은 여기 넣지 않는다 |
...
한때 "채택곡의 원본만 `sources/`에. 마스터링 소스는 저장소 안에 있어야 한다"고 적혀
있었다. **그 방침은 쓰지 않는다.**

원본은 **생성한 Gemini 세션과 작업자 로컬에 남아 있다.** 저장소에 없어도 사라지지 않는
것을 위해 곡당 4MB를 히스토리에 영구히 박을 이유가 없다. 재마스터링이 필요하면 그때
로컬에서 꺼내 오면 된다. 빈도가 낮은 일에 상시 비용을 치르는 거래다.

`sources/`에 `city_the-center-of-the-room.mp3` 하나가 남아 있는 것은 그 방침이 있던
시절의 잔재다. **지워도 클론은 줄지 않으므로**(blob이 이미 히스토리에 있다) 굳이 건드리지
않을 뿐이고, **따라 해야 할 예시가 아니다.**

title·alley 채택본의 원본이 `sources/`에 없는 것도 누락이 아니라 **의도**다. 스테이지
3~5도 같다.
...
하지 않는다** — 자르고 크로스페이드를 걸면 세대 손실이 쌓인다. 저장소에 원본이 없으므로
**가공할 때는 로컬 원본을 가져와서 시작한다.**
