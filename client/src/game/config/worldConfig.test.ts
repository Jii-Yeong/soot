...
describe('getParallaxLayerWidth', () => {
  ...
  it('creates a layer whose travel matches the requested factor', () => {
    ...
    expect(layerWidth).toBe(3245.75);
...
describe('continuous stage dimensions', () => {
  it('maps two 200-unit combat rooms plus a 100-unit boss room into one stage', () => {
    expect(VIEWPORT_LENGTH_UNITS).toBe(70);
    expect(ROOM_LENGTH_UNITS).toBe(200);
    expect(BOSS_ROOM_LENGTH_UNITS).toBe(100);
    expect(STAGE_LENGTH_UNITS).toBe(500);
    expect(ROOMS_PER_STAGE).toBe(3);
    expect(STAGE_WORLD_WIDTH).toBe(ROOM_WORLD_WIDTH * 2 + BOSS_ROOM_WORLD_WIDTH);
  });
});
